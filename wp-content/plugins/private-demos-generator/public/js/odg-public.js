jQuery(document).ready(function ($) {
    ...
    $.inlineValidationEngine = function( elem, options, arg ) {        
        ...
        if(error_messages_area)
        {
            ...
            //On dtecte lorsque la zone de notification change
...
//            =$("#"+e.target.id).attr("data-form-failure-messages");
	...
	$("#demo-request-frm, .demo-request-frm").inlineValidationEngine({
    ...
    /**
     * Correção para funcionar com classes
     * Permitindo vários itens numa única tela
     */
    jQuery( ".demo-request-frm" ).on( 'submit', function( e ){

        elem = this;
        
        jQuery.ajax({
            type: "POST",
            url: odg_ajax_object.ajax_url,
            dataType: 'json', 
            data: { 
                name: jQuery( elem ).find( ".demo-request-name" ).val(), 
                email: jQuery( elem ).find( ".demo-request-email" ).val(),
                action: "pdg_generate_demo_install", 
                demo_id: jQuery( elem ).find( ".demo-id" ).val()
                },
    ...
    function process_demo_install( form, status )
    {
            ...
            if(status===true){
                ...
                var nome=$("#demo-request-nome").val();
                var telefone=$("#demo-request-telefone").val();
